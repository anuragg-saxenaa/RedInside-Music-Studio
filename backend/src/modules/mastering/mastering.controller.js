...
export const MasteringController = {
        ...
        for (let i = 0; i < ids.length; i++) {
          ...
          try {
            if (isMusicId) {
              ...
              const outputPath = path.join(mastersDir, `${id}_spotify_master.flac`);
              ...
              const r2MasterKey = `projects/${pid}/masters/${id}_spotify_master.flac`;
              try {
              ...
              const r2MasterKey = `projects/${projectId}/masters/${id}_spotify_master.flac`;
              await storage.saveAudioFile(fs.readFileSync(outputPath), r2MasterKey);
  ...
  async download(req, res, next) {
    try {
      ...
      const mastersDir = storage.getMastersDir(projectId);
      const filePath = path.join(mastersDir, `${fileId}_spotify_master.flac`);
  ...
  async saveToMusic(req, res, next) {
    try {
      ...
      for (const fileId of fileIds) {
        ...
        const r2MasterKey = `projects/${projectId}/masters/${fileId}_spotify_master.flac`;
  ...
  async downloadZip(req, res, next) {
    try {
      ...
      for (const fileId of ids) {
        ...
        const masterFile = masterFiles.find(f => f.endsWith('_spotify_master.flac'));
