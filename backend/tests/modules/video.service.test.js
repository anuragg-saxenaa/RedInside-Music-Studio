...
test('should create video model', async () => {
  const project = ProjectModel.create({ name: 'Test Video Project' });
...
test('should get video by id', async () => {
  ...
  // Non-existent video should return undefined
  const result = videoService.getVideo('non-existent-id');
  assert.strictEqual(result, undefined);
});

test('should get project videos', async () => {
  const project = ProjectModel.create({ name: 'Test Project Videos' });
  const videoService = new VideoService();

  // Should return empty array for project with no videos
  const result = videoService.getProjectVideos(project.id);
