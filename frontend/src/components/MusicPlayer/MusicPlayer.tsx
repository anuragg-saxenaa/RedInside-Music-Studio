            ...
            {musicHistory.map(music => (
              <React.Fragment key={music.id}>
                ...
                {editingMusic?.id === music.id && (
                  <div ref={editorRef} style={{ marginTop: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
                      <button
                        onClick={() => setEditingMusic(null)}
                        style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 6, padding: '6px 14px', color: 'rgba(255,255,255,0.4)', fontSize: 11, cursor: 'pointer', letterSpacing: '0.06em' }}
                      >
                        ✕ Close Editor
                      </button>
                    </div>
                    <AudioEditorPanel
                      ...
                      musicId={editingMusic.id}
                      onExport={() => { setEditingMusic(null); fetchMusicList(); }}
                    />
                  </div>
                )}
              </React.Fragment>
